  ...
  render() {
    ...
    if (!this.props.newGame) {
      ...
      return React.createElement("div", {
        ...
        label: "Game",
        default_value: this.props.gameId,
        onChange: this.setGameId,
        mutable: true
      }), React.createElement(FieldEntry, {
        label: "Your Word",
...
class JoinableGamesList extends React.Component {
  ...
  render() {
    const games_li = this.props.games.map(game => React.createElement("li", {
      key: game,
      onClick: () => {
        this.onNew(game);
      }
...
class SideBar extends React.Component {
  render() {
    ...
    if (this.props.loggedIn) {
      ...
      to_join_list = React.createElement(JoinableGamesList, {
        onNewGame: this.props.onNewGame,
...
class BagelBody extends React.Component {
  ...
  createUser() {
    request_create_user(this.state.username, this.state.password).then(res => {
      if (res.valid) {
        localStorage.setItem('username', this.state.username);
        localStorage.setItem('password', this.state.password);
        this.setUserId(res['user_id']);
  ...
  render() {
    ...
    if (this.state.activeGame != null) {
      ...
      gamePanel = React.createElement(GamePanel, {
        game: {
          game_id: this.state.activeGameId
        },
