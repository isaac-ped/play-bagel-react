...
class ScratchPanel extends React.Component {
  constructor(props) {
    ...
    if (this.props.game != null) {
      ...
      console.log("Stored contents", stored_contents);
  ...
  saveText(e) {
    console.log("DFADSFADS");
    this.setState({
      contents: e.target.value
    });
    console.log(this.props.game, e.target.value, "SAVED");
...
class BagelBody extends React.Component {
  ...
  doGuess(guess) {
    request_guess_word(this.state.activeGame.game_id, this.state.username, this.state.userId, guess).then(res => {
      if (res.valid) {
        this.setGameList(res['games']);
      } else {
